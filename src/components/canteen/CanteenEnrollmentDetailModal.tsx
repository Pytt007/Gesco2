...
import { CanteenEnrollment } from '../../services/canteen/types';
import {
  X, UtensilsCrossed, User, Phone, DollarSign, Calendar, CheckCircle2,
  AlertCircle, Edit2, Trash2
} from 'lucide-react';

interface CanteenEnrollmentDetailModalProps {
  isOpen: boolean;
  enrollment: CanteenEnrollment | null;
  onClose: () => void;
  onEdit?: (enrollment: CanteenEnrollment) => void;
  onDelete?: (enrollment: CanteenEnrollment) => void;
}

export const CanteenEnrollmentDetailModal: React.FC<CanteenEnrollmentDetailModalProps> = ({
  isOpen,
  enrollment,
  onClose,
  onEdit,
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 10, fontSize: '0.8125rem', color: '#a7f3d0', marginTop: 3 }}>
                <span>{enrollment.matricule}</span>
                <span>•</span>
                <span>{enrollment.className}</span>
                <span>({enrollment.levelCode})</span>
              </div>
            </div>
          </div>
          <button
        ...
        <div style={{ padding: '20px 24px', overflowY: 'auto', flex: 1, display: 'grid', gap: 18 }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: '#d1fae5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#059669' }}>
              ...
              <div>
                ...
                <div style={{ fontWeight: 800, color: '#064e3b', fontSize: '1rem' }}>
                  Niveau {enrollment.levelCode}
                </div>
                <div style={{ fontSize: '0.8125rem', color: '#059669' }}>
                  Année : {enrollment.academicYearId}
                </div>
              </div>
            </div>
            <div>
              ...
              >
                ...
                {isPaid ? 'À jour' : isPartial ? 'Partiel' : 'Impayé'}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10 }}>
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 14px' }}>
              ...
              <div style={{ fontSize: '1rem', fontWeight: 700, color: '#0f172a', marginTop: 2 }}>
                {enrollment.annualRate.toLocaleString('fr-FR')} F
              </div>
            </div>
            ...
            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 14px' }}>
              <div style={{ fontSize: '0.75rem', color: '#0284c7', fontWeight: 600 }}>Net à payer</div>
              <div style={{ fontSize: '1rem', fontWeight: 700, color: '#0369a1', marginTop: 2 }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8125rem', marginBottom: 6 }}>
              ...
              <span style={{ fontWeight: 700, color: isPaid ? '#16a34a' : '#10b981' }}>{pct}%</span>
            </div>
            <div style={{ height: 8, background: '#e2e8f0', borderRadius: 99 }}>
              <div
                style={{
                  height: '100%',
                  width: `${pct}%`,
                  background: isPaid ? '#16a34a' : 'linear-gradient(90deg, #10b981, #064e3b)',
                  borderRadius: 99,
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <h6 style={{ margin: 0, fontWeight: 700, fontSize: '0.875rem', color: '#0f172a', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Calendar size={15} color="#10b981" /> Échéancier ({enrollment.periods.length} échéance{enrollment.periods.length > 1 ? 's' : ''})
              </h6>
            </div>
